...
result_dir=stage4_retry2_results
started=0
failure_reported=0
...
for tool in awk basename cp git mkdir sed sha256sum; do
...
if [[ -f "$generated/retry2_source_commit.txt" ]]; then
  ...
  git -C "$source_repo" fetch --no-tags "$gerrit_url" "refs/heads/$gerrit_branch" \
    >"$logs/step5_gerrit_fetch.txt" 2>&1
...
remote_head_now=$(git ls-remote "$gerrit_url" "refs/heads/$gerrit_branch" | awk '{print $1}')
[[ "$remote_head_now" == "$source_start_commit" ]] ||
  step_fail "Gerrit toolchain HEAD changed before push"
git -C "$source_repo" push "$gerrit_url" \
  "HEAD:refs/heads/$gerrit_branch" >"$logs/step5_gerrit_push.txt" 2>&1 ||
  step_fail "Gerrit push failed"
published_head=$(git ls-remote "$gerrit_url" "refs/heads/$gerrit_branch" | awk '{print $1}')
